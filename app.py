...
# NBK Pay - Secure Card-to-Crypto POS Terminal Backend

app = Flask(__name__)
app.secret_key = 'nbkpay_super_secret_key'  # Use an environment variable for production!
...
# Ensure password file exists with a secure default
if not os.path.exists(PASSWORD_FILE):
    with open(PASSWORD_FILE, "w") as f:
        hashed = hashlib.sha256("NBKPay@2025".encode()).hexdigest()
...
# Demo card database (replace with real API in production)
DUMMY_CARDS = {
...
def login():
    if request.method == 'POST':
        ...
        if user == ADMIN_USERNAME and check_password(passwd):
            session['logged_in'] = True
            flash("Welcome to NBK Pay!", "success")
            return redirect(url_for('protocol'))
        flash("Invalid username or password.", "danger")
    return render_template('login.html', brand="NBK Pay")
...
def change_password():
    if request.method == 'POST':
        ...
        if not check_password(current):
            return render_template('change_password.html', error="Current password incorrect.", brand="NBK Pay")
        set_password(new)
        return render_template('change_password.html', success="Password changed.", brand="NBK Pay")
    return render_template('change_password.html', brand="NBK Pay")
...
def protocol():
    ...
    return render_template('protocol.html', protocols=PROTOCOLS.keys(), brand="NBK Pay")
...
def amount():
    if request.method == 'POST':
        ...
        if not amount or not amount.isdigit():
            flash("Invalid amount. Please enter numbers only.", "danger")
            return render_template('amount.html', brand="NBK Pay")
    ...
    if request.method == 'POST':
        ...
        if method == 'ERC20':
            ...
            if not wallet.startswith("0x") or len(wallet) != 42:
                flash("Invalid ERC20 address format.", "danger")
                return redirect(url_for('payout'))
            ...
            if not wallet.startswith("T") or len(wallet) < 34:
                flash("Invalid TRC20 address format.", "danger")
                return redirect(url_for('payout'))
            session['wallet'] = wallet
        else:
            flash("Please select a payout method.", "danger")
            return redirect(url_for('payout'))

        return redirect(url_for('card'))

    return render_template('payout.html', brand="NBK Pay")
...
def card():
    ...
    return render_template('card.html', brand="NBK Pay")
...
def auth():
    ...
    if request.method == 'POST':
        ...
        if len(code) != expected_length:
            return render_template('auth.html', warning=f"Code must be {expected_length} digits.", brand="NBK Pay")
        if code == card['auth']:
            # TODO: Integrate real crypto payout API here for production
    ...
    return render_template('auth.html', brand="NBK Pay")
...
@login_required
def offline():
    return render_template('offline.html', brand="NBK Pay")

if __name__ == '__main__':
    # For Render, use 'gunicorn app:app' in your start command
    app.run(debug=True)